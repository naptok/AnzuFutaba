    ...
    if (data.index < data.count) {
      ...
      if (!(_array[0].startsWith('#'))) {
        try {
          ...
          (require(`./${_array[0]}.js`))(message, _array, client, embed, port, () => { setTimeout(() => { running(data) }, 700) })
        } catch (e) {
          _str2 = '**500 ERROR**'
          _str = '명령어를 잘못 입력하셨습니다'
          message.channel.send(`${message.author} ${_str2}\n\`\`\`\n${_str}\n\`\`\``)
