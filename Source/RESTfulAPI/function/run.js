module.exports = (body, moduleControl) => {
  ...
  var port_string
  if (body.port == undefined) { port_string = '' } else { port_string = `-p ${(body.port).replace('-', ':')}` }

  var name_string
  if (body.name == undefined) { name_string = '' } else { name_string = `--name ${body.name}${getRandomInt(10000, 99999)}` }

  var directory
  if (body.directory == undefined) { directory = '' } else { directory = `--privileged=true -v ${(body.directory).replace('-', ':')}` }
